...
def Kalman_filter(y, H, Z, R, mean_u, T, Q, omega, beta=0, SV=[0]):
    ...
    K = np.zeros(I)
    for i in range(I):
        v[i] = y[i] - Z[i] * a[i] - mean_u - beta * SV[i]
...
def fig_14_5_ii_p2(x, alpha_hat_adj, a):
    plt.plot(x, linestyle="", marker="o", color='grey', markersize=2)
    plt.plot(alpha_hat_adj, color='k', label='alpha')
    plt.plot(a, label='a', color='blue', alpha=0.625)
    plt.legend()
...
def fig_assignment_d(a, alpha_hat):
    plt.plot(alpha_hat, color='k', label='alpha')
    plt.plot(a, label='a', color='blue', alpha=0.625)
    plt.legend()
    plt.show()

def get_a_and_alpha(para_est, x, y, I, RV):
    sig2_u = (np.pi ** 2) / 2  # u = log(eps_t**2)
    mean_u = -1.27
    ...
    return a, alpha_hat, beta

def calc_H(alpha, phi, omega):
    xi = omega/(1-phi)
    H = alpha - xi
    return H
...
def smoothing(para_est, x, y, I, RV):
    mean_u = -1.27
    a, alpha, beta = get_a_and_alpha(para_est, x, y, I, RV)

    altered_a = np.zeros(I)
    altered_alpha = np.zeros(I)
    for i in range(I):
        addition = 0
        if len(para_est) == 4:
            addition = beta * RV[i]
        altered_a[i] = a[i] + mean_u + addition
        altered_alpha[i] = alpha[i] + mean_u + addition

    fig_assignment_d(altered_a, altered_alpha)
    fig_14_5_ii_p2(x, altered_alpha, altered_a)
...
def main():
    ...
    print('\n\n\t\t\t-----GBP_USD-----\n\n')
    GBP_USD_data = loadDatasv('sv.dat')
    GBP_USD_y = np.array(GBP_USD_data) / 100
    GBP_USD_x = make_linear(GBP_USD_y)
    runSV(GBP_USD_y, GBP_USD_x, theta_ini, theta_bnds, options)

    print('\n\n\t\t\t-----SP_500-----\n\n')
    SP_500_data = loadDataOxford('SPX_2012_now.csv', columnOfData=5)
    SP_500_y = convertToLogDiff(SP_500_data)
    SP_500_x = make_linear(SP_500_y)
    runSV(SP_500_y, SP_500_x, theta_ini, theta_bnds, options)

    print('\n\n\t\t\t-----SP_500 + extension-----\n\n')
    RV5 = loadDataOxford('SPX_2012_now.csv', columnOfData=11)
    logRV5 = np.log(np.array(RV5))
    theta_ini.append(1)
    theta_bnds.append((-np.inf, np.inf))
    runSV(SP_500_y, SP_500_x, theta_ini, theta_bnds, options, RV=logRV5)
